...
@app.route('/')
def exercise1():
    message = "Hi, " + current_user.get_full_name() + "!"
    return message 
...
def exercise4():
    ...
    img = restaurants[0].get('image_url')
    addr = restaurants[0].get('display_address')
    price = restaurants[0].get('price')
    rating = restaurants[0].get('rating')
    reviews = restaurants[0].get('review_count')
    return render_template(
        'restaurant.html',
        user=current_user,
